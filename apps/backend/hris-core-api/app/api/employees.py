...
from app.services.persona_policy import (
    can_view_team_employee_roster,
    can_view_tenant_employee_roster,
...
def list_team_employees(
    ...
    if not can_view_team_employee_roster(user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Team roster access requires line manager or higher privileges.",
        )
    mapping = get_tenant_mapping(user.tenant_id)
    return srms_client.list_team_employees(
        mapping,
        user.raw_token,
