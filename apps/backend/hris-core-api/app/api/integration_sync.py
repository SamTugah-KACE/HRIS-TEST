...
from app.services.enrollment_jobs import enqueue_enrollment
...
@router.post("/enrollment/preview", status_code=status.HTTP_202_ACCEPTED)
def preview_federated_enrollment(
    request: Request,
    tenant_id: Optional[str] = None,
    max_users: int = Query(0, ge=0),
    user: AuthenticatedUser = Depends(require_roles("hris:super_admin")),
):
    return enqueue_enrollment(actor=user, tenant_id=tenant_id, mode="discover", max_users=max_users)


@router.post("/enrollment/apply", status_code=status.HTTP_202_ACCEPTED)
def apply_federated_enrollment(
    request: Request,
    tenant_id: Optional[str] = None,
    max_users: int = Query(0, ge=0),
    user: AuthenticatedUser = Depends(require_roles("hris:super_admin")),
    ...
    if not settings.enable_federated_keycloak_sync:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Enrollment apply is disabled")
    return enqueue_enrollment(actor=user, tenant_id=tenant_id, mode="apply", max_users=max_users)


@router.get("/enrollment/jobs")
def list_federated_enrollment_jobs(
    limit: int = Query(50, ge=1, le=200),
    user: AuthenticatedUser = Depends(require_roles("hris:super_admin")),
):
    return {"jobs": automation_store.list_enrollment_jobs(limit)}


@router.get("/enrollment/jobs/{job_id}")
def get_federated_enrollment_job(
    job_id: str,
    user: AuthenticatedUser = Depends(require_roles("hris:super_admin")),
):
    job = automation_store.get_enrollment_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Enrollment job not found")
    return job


@router.post("/enrollment/jobs/{job_id}/retry", status_code=status.HTTP_202_ACCEPTED)
def retry_federated_enrollment_job(
    job_id: str,
    user: AuthenticatedUser = Depends(require_roles("hris:super_admin")),
):
    job = automation_store.retry_enrollment_job(job_id)
    if not job:
