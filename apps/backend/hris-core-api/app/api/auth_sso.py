...
def _resolve_callback_url(request: Request) -> str:
    configured = str(get_settings().auth_sso_callback_url or "").strip()
    return configured or str(request.url_for("sso_callback"))
...
def sso_start(request: Request, next_path: Optional[str] = Query(None, alias="next")):
    ...
    enforce_rate_limit(
        request,
        scope="auth_sso_start",
        limit=settings.rate_limit_auth_start_max,
    )
...
def sso_refresh(request: Request):
    ...
    enforce_rate_limit(
        ...
        limit=settings.rate_limit_auth_refresh_max,
...
def sso_logout(request: Request):
    ...
    enforce_rate_limit(
        ...
        limit=settings.rate_limit_auth_logout_max,
