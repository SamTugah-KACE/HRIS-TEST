...
def _derive_modules(data: dict) -> TenantModules:
    ...
    if bool(settings.enable_all_modules_by_default):
        # Platform default: all modules are enabled unless tenant has explicit selective modules config.
        default_active = ModuleStatus(status="active", ready=True, configured=True)
        return TenantModules(
            **{
                "srms": default_active.model_dump(),
                "eappraisal": default_active.model_dump(),
                "eleave": default_active.model_dump(),
            }
        )
