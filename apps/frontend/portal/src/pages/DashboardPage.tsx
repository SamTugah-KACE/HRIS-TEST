...
import { HRIS_ROLES, getRoleLabel, hasMinimumRole, isManagerRole } from '../auth/roles';
...
export const DashboardPage: React.FC = () => {
  ...
  const isTenantManager = hasMinimumRole(role, HRIS_ROLES.HR_MANAGER) && !isSuperAdmin;
  const isLineManager = role === HRIS_ROLES.LINE_MANAGER;
  ...
  const hasSrms = enabledModuleIds.has('srms');
  const hasEappraisal = enabledModuleIds.has('eappraisal');
  const hasEleave = enabledModuleIds.has('eleave');
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="mb-4 text-lg font-semibold text-gray-900">
          {isTenantManager || isLineManager ? 'HR Modules' : 'My Services'}
        </h2>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {isTenantManager || isLineManager ? (
            <>
              {hasSrms && (
                <ModuleCard
                  title="Staff Records"
                  ...
                  icon={Users}
                  href={role === HRIS_ROLES.LINE_MANAGER ? '/employees/team' : '/employees'}
                  color="blue"
                  nativeModuleId="srms"
                  stats={[
            ...
            <>
              <ModuleCard
                ...
                stats={[
                  { label: 'Department', value: employeeDepartment },
                  { label: 'Branch', value: employeeBranch },
                  { label: 'Staff ID', value: employeeStaffId },
                  { label: 'Status', value: employeeStatus },
                ]}
              />
              {hasEappraisal && (
                <ModuleCard
                  ...
                  nativeModuleId="eappraisal"
                  stats={[
                    { label: 'Current Cycle', value: String(appraisalData?.employee.current_cycle.title ?? 'Current Cycle') },
                    { label: 'My Score', value: String(appraisalData?.employee.past_appraisals?.[0]?.score ?? 'N/A') },
                    { label: 'Status', value: `${appraisalProgress}% Complete` },
                    { label: 'Due Date', value: String(appraisalData?.employee.current_cycle.due_date ?? 'N/A') },
                  ]}
                />
              )}
              {hasEleave && (
                <ModuleCard
                  ...
                  nativeModuleId="eleave"
                  stats={[
                    { label: 'Annual Balance', value: `${annualAvailable} days` },
                    { label: 'Used This Year', value: `${annualUsed} days` },
                    { label: 'Pending', value: `${annualPending} request(s)` },
                    { label: 'Sick Leave', value: `${Number(employeeBalances.find((b) => String(b.type).toLowerCase().includes('sick'))?.total ?? 0)} days` },
                  ]}
                />
              )}
            </>
