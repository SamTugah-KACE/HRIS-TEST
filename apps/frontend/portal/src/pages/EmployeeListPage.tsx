...
import { getEmployees, getTeamEmployees, type EmployeeListResponse } from '../api/hrisCoreClient';
import { useAuth } from '../auth/AuthProvider';
import { hasMinimumRole, isManagerRole, HRIS_ROLES } from '../auth/roles';
...
export const EmployeeListPage: React.FC = () => {
  ...
  const isTeamRoute = location.pathname === '/employees/team';
  const canViewFullRoster = hasMinimumRole(role, HRIS_ROLES.HR_MANAGER);
  ...
  useEffect(() => {
    ...
    if (!canViewFullRoster && !isTeamRoute) {
      navigate('/employees/team', { replace: true });
    }
  }, [canViewFullRoster, isTeamRoute, role, navigate]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const fetcher = isTeamRoute ? getTeamEmployees : getEmployees;
      const result = await fetcher({ search, status: statusFilter, department: departmentFilter, page, page_size: 10 });
      setData(result);
    } catch {
      setData(null);
    } finally {
  ...
  }, [departmentFilter, isTeamRoute, page, search, statusFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ModuleNativeLaunchBanner moduleId="srms" />

      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{isTeamRoute ? 'Team Directory' : 'Staff Records'}</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {isTeamRoute ? 'View staff records scoped to your team' : 'Browse and manage employee records from SRMS'}
          </p>
        </div>
        {canViewFullRoster && !isTeamRoute && (
